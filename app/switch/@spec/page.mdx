...
|---------------------------------------- | ---------------------------- | ------------------------- | ------------------- |
| `--par-color-bg-switch-track-enabled`   | Switch track background      | Enabled                   | `.switch-track:enabled` |
| `--par-color-bg-switch-track-hovered`   | Switch track background      | Hovered                   | `.switch-track:hover` |
| `--par-color-bg-switch-track-disabled`  | Switch track background      | Disabled                  | `.switch-track:disabled` |
| `--par-color-bg-switch-track-checked`   | Switch track background      | Checked                   | `.switch-track.checked` |
| `--par-color-bg-switch-track-invalid`   | Switch track background      | Invalid/Error             | `.switch-track.invalid` |
| `--par-color-border-switch-track`       | Switch track border          | Any                       | `.switch-track` |
| `--par-color-thumb-switch-enabled`      | Switch thumb                 | Enabled                   | `.switch-thumb:enabled` |
| `--par-color-thumb-switch-hovered`      | Switch thumb                 | Hovered                   | `.switch-thumb:hover` |
| `--par-color-thumb-switch-disabled`     | Switch thumb                 | Disabled                  | `.switch-thumb:disabled` |
| `--par-color-thumb-switch-checked`      | Switch thumb                 | Checked                   | `.switch-thumb.checked` |
| `--par-color-thumb-switch-invalid`      | Switch thumb                 | Invalid/Error             | `.switch-thumb.invalid` |
| `--par-color-icon-switch`               | Switch thumb icon            | Any                       | `.switch-thumb .icon` |

> _Note: Switch uses the same design tokens as the Input or Toggle component. See Input/Toggle documentation for available tokens._
...
| **color**         | string                    |             | Switch color                      |
| **size**          | 'sm' \| 'md' \| 'lg'      | _'md'_      | Switch size                       |
| **...InputProps** | [See Input](../BaseInput) |             | Inherits all props from **Input** |

> **_Note:_** _Switch_ **inherits** all props from **Input** and the native **input** element. See [Input documentation](../BaseInput) for full details on available props and tokens.
